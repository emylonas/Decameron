...
<xsl:variable name="persRefs" select="descendant::person[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), $targetPerson)]/@id"/>
...
<xsl:template match="/">
 <xsl:variable name="hitCounts" select="count(descendant::name[@type='person' and @persref=$persRefs])"/>
   <xsl:choose>
    <xsl:when test="$hitCounts!=0">
      <div id="search_results">
      <xsl:for-each select="$persRefs">
        <xsl:variable name="ref" select="."/>
        <xsl:variable name="persName" select="/descendant::person[@id=$ref]/text()"/>
        <xsl:variable name="hitCount" select="count(/descendant::name[@type='person' and @persref=$ref])"/>
       <h3><span class="highlight"><xsl:value-of select="$persName"/></span> (Total: <xsl:value-of select="$hitCount"/>)</h3>
       	<ul>
         <xsl:call-template name="hitList">
           <xsl:with-param name="persRef" select="$ref"/>
         </xsl:call-template>
      </ul>
      </xsl:for-each>
      </div>
      <xsl:choose>
       <xsl:when test="$displayID='none'">
        <xsl:apply-templates select="//div2[descendant::name/@persref=$persRefs]" mode="firstDisplay"/>
       </xsl:when>
       <xsl:otherwise>
    ...
    <xsl:otherwise>
      <h3><span class="highlight"><xsl:value-of select="$targetPerson"/></span> not found</h3>
    </xsl:otherwise>
   </xsl:choose>
</xsl:template>
...
<xsl:template name="hitList">
  <xsl:param name="persRef"/>
  <xsl:for-each select="//div2[descendant::name/@persref=$persRef]">
   ...
   <xsl:variable name="hitsNum" select="count(descendant::name[@persref=$persRef])"/>
   <xsl:choose>
    <xsl:when test="position()=1 and $displayID='none'">
     <li><span class="current"><a href="search.php?q={$targetPerson}&amp;type=person&amp;display={$div2ID}&amp;work=Dec&amp;lang={$langID}"><xsl:value-of select="$div1Head"/>,<xsl:text>  </xsl:text><xsl:value-of select="$div2Head"/></a>: <xsl:value-of select="$hitsNum"/></span></li>
    </xsl:when>
    <xsl:when test="$displayID=$div2ID">
     <li><span class="current"><a href="search.php?q={$targetPerson}&amp;type=person&amp;display={$div2ID}&amp;work=Dec&amp;lang={$langID}"><xsl:value-of select="$div1Head"/>,<xsl:text>  </xsl:text><xsl:value-of select="$div2Head"/></a>: <xsl:value-of select="$hitsNum"/></span></li>
    </xsl:when>
    <xsl:otherwise>
     <li><a href="search.php?q={$targetPerson}&amp;type=person&amp;display={$div2ID}&amp;work=Dec&amp;lang={$langID}"><xsl:value-of select="$div1Head"/>,<xsl:text>  </xsl:text><xsl:value-of select="$div2Head"/></a>: <xsl:value-of select="$hitsNum"/></li>
    </xsl:otherwise>
   </xsl:choose>
  </xsl:for-each>
</xsl:template>
...
<!-- <h2><xsl:apply-templates mode="display"/></h2> -->
</xsl:template> 
...
<xsl:template match="lg" mode="display">
  <xsl:if test="position()= 1">
   <h3>[Voice: <xsl:value-of select="parent::div3/@who"/>]</h3>
  </xsl:if>
  <p><xsl:apply-templates mode="display"/></p>
</xsl:template>
...
<xsl:template match="name" mode="display">
 <xsl:choose>
  <xsl:when test="@persref=$persRefs">
